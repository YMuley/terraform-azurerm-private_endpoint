...
for_each = local.private_endpoint
  ...
  #------ web app
  dynamic "private_service_connection" {
    for_each = each.value.resource_type == "web_app" ? [{}] : []
    content {
      name                           = "private_service_connection_${each.value.name}"
      is_manual_connection           = false
      private_connection_resource_id = var.windows_web_app_output[each.value.resource_name].id
      subresource_names              = each.value.subresource_name    
    }
  }
  ...
  #------ redis cache
  dynamic "private_service_connection" {
    for_each = each.value.resource_type == "redis_cache" ? [{}] : []
    content {
      name                           = "private_service_connection_${each.value.name}"
      is_manual_connection           = false
      private_connection_resource_id = var.redis_cache_output[each.value.resource_name].id
      subresource_names              = each.value.subresource_name    
    }
  }
